/*******************************************************************************
*
* Computation of the mesonic spectrum with Gaussian and HYP smearing
*
*******************************************************************************/
...
#define init_input_mesons(varname)                                                                                        \
    {                                                                                                                     \
        .read = {                                                                                                         \
            ...
            { "enable default semwall w/ Gaussian smearing", "mes:def_semwall_smeared = %d", INT_T, &(varname).def_semwall_smeared },                          \
            ...
            { "n_smr", "mes:n_smr = %d", INT_T, &(varname).n_smr },                                                       \
            { "alpha", "mes:alpha = %lf", DOUBLE_T, &(varname).alpha },                                                   \
...
int main(int argc, char *argv[]) {
    ...
    if (strcmp(list_filename, "") != 0) {
        error((list = fopen(list_filename, "r")) == NULL, 1, "main [measure_spectrum_smeared.c]", "Failed to open list file\n");
    }
...
#else
                error(1, 1, "main [measure_spectrum_smeared.c]", "contract_baryon not implemented for NG!=3");
#endif
            }
